...
export const fetchTrackDetailsFromMongoDB = async (trackIds: string[]): Promise<TrackDetails[]> => {
  try {
    // Try to fetch from MongoDB backend API first
    const promises = trackIds.map(async (id) => {
      try {
        const response = await fetch(API_ENDPOINTS.TRACK_DETAILS(id));
        if (response.ok) {
          const data = await response.json();
          return {
            id: id,
            title: data.title || data.name || `Track ${id}`,
            artist: data.artist || data.artist_name || "Unknown Artist",
            album: data.album || data.album_name || "Unknown Album",
            genre: data.genre || data.genre_top,
            mood: data.mood || data.emotion,
            keywords: data.keywords || (data.genre ? [data.genre] : ["Music"]),
            duration: data.duration || "03:00",
            thumbnail: data.thumbnail || "🎵",
            producer: data.producer,
            writer: data.writer,
            licensing: data.licensing || "Standard",
          };
        }
      } catch (error) {
        console.error(`Error fetching track ${id} from MongoDB:`, error);
        return null;
      }
      return null;
    });
    ...
    const tracks = results.filter((track): track is TrackDetails => track !== null);
    
    // If we got some tracks from MongoDB, return them
    if (tracks.length > 0) {
      return tracks;
    }
  } catch (error) {
